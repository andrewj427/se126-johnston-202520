...
def again():  # <--- FUNCTION HEADER
    '''this function asks a user if theyd like to enter another temp, checks the response for validity, and then returns a valid response back to the main program'''

    ans = input("\t\tWould you like to enter another temperature? [y/n]: ").lower()

    #while loop trap - ensure user provides valid valuse before moving on
...
while (answer == "y" or answer == "Y") and temp_total >=0:
    ...
    answer = again() #the return value of this function will replace this call as final result for 'answer'
